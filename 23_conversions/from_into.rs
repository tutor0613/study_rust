...
impl From<&str> for Person {
    ...
    //     if items.len() == 2 && !items[0].is_empty() && items[1].parse::<u8>().is_ok() {
    //         person.name = items[0].to_string();
    //         person.age = items[1].parse::<u8>().unwrap();
    //     }
    ...
    fn from(s: &str) -> Self {
        ...
        let (Some(name), Some(age), None) = (items.next(), items.next(), items.next()) else {
            return Self::default();
        };
